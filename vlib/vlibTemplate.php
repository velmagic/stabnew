...
if (!defined('vlibTemplateClassLoaded')) {
     ...
     * For instructions on how to use vlibTemplate, see
    ...
    \-----------------------------------------------------------------------------*/
        ...
        function vlibTemplate ($tmplfile=null, $options=null) {
            ...
            $vLibIni_obj = new vlibIni();

            if (is_array($vLibIni_obj->vlibTemplate())) {
                foreach ($vLibIni_obj->vlibTemplate() as $name => $val) {
...
//                },$data);
                ...
                $data = preg_replace_callback($regex, array($this, 'callback'), $data);
