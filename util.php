...
function getDeliveryArrayPHP5( $xpath_expression = '//delivery' )
{
  ...
  foreach($nodes as $node)
  {
   ...
   //   $delivery[$subnode->nodeName] = iconv("UTF-8", "windows-1251",$subnode->nodeValue);
      $delivery[$subnode->nodeName] = $subnode->nodeValue;
    }
...
function getProductsArray( $xpath_expression = '//product' )
{
  if (version_compare(PHP_VERSION, '5.0.0', '>=')) 
  {
    $res = getProductsArrayPHP5($xpath_expression);
  } else {
    $res = getProductsArrayPHP4($xpath_expression);
  }
  foreach ($res as &$item) {
    if (isset($item['composite']) && (is_array($item['composite'])))
    {
      $item['price'] = 0;
      $item['weight'] = 0;
      $item['available'] = 1;
      foreach($item['composite'] as $subprod)
      {
        $prod = getProductsArray("//product[@code='" . $subprod['code'] . "']");
        if ((is_array($prod)) && (count($prod) > 0)) {
          $item['price'] += $prod[0]['price'] * $subprod['quantity'];
          $item['weight'] += $prod[0]['weight'] * $subprod['quantity'];
          $item['available'] = (int)((boolean)$item['available'] && (boolean)$prod[0]['available']);
        }
      }
      $item['fprice'] = number_format($item['price'], 0, '.', ' ');
    }
  }
  return $res;
}
...
function getProductsArrayPHP5( $xpath_expression = '//product' )
{
  ...
  foreach($nodes as $node)
  {
    ...
    foreach($node->childNodes as $subnode) {
      ...
      if (($subnode->nodeName == 'characteristics') || ($subnode->nodeName == 'composite'))
      {
        $sub2nodes = array();
        foreach($subnode->childNodes as $sub2node)
        {
	      ...
	      {
            $attrib = $sub2node->attributes;
            $length = $attrib->length;
            $attrs = array();
            for ($i = 0; $i < $length; ++$i) {
              $name = $attrib->item($i)->name;
              $value = $attrib->item($i)->value;
  //            $attrs[$name] = iconv("UTF-8", "windows-1251", $value);
              $attrs[$name] = $value;
            }
            $sub2nodes[] = $attrs;
	      }
     ...
     //   $product[$subnode->nodeName] = iconv("UTF-8", "windows-1251",$subnode->nodeValue);
        $product[$subnode->nodeName] = $subnode->nodeValue;
      }
    }
